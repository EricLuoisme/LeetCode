...
def longestPalindrome(s):
    ...
    while palin_in_len <= total_len:
        for i in range(total_len - palin_in_len + 1):
            j = i + palin_in_len - 1 # j is the exactly 'end' character we need to check, i is the 'start'
            if s[i] == s[j] and table[i+1][j-1] == True:
                # table[i+1][j-1] means to check the string between i and j is a palidrome or not
        ...
        palin_in_len += 1 # find a bigger palindrome

    return s[start_index : start_index + max_length] # we know where the palidrome start, can get it with it's length
